...
/**
 * This represents a CardContentBuilder implementation that creates PieChart type
 * Card Content. This allows us to dynamically allocate a card to have a bar chart
 * using the `<pie-chart></pie-chart>` selector without hard-coding it into the content.
 *
 * @see BaseChartContentBuilder
 */
export class PieChartContentBuilder extends BaseChartContentBuilder {

  /**
   * Builds the Content Object in this case a PieChartComponent
   */
  buildCardContent(): CardContentObject {
    // dynamically create and instert our content component using the supplied
    // component factory.
    let componentFactory: ComponentFactory<CardContentObject> = this.componentFactoryResolver.resolveComponentFactory(PieChartComponent);
...
export class PieChartComponent extends BaseChartComponent implements CardContentObject {
